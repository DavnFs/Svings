...
| `supabase/migrations/20260101000002_rls_policies.sql` | Row Level Security (profiles, transactions, raw_emails) |
| `supabase/migrations/20260101000003_accounts.sql` | `accounts` table, `transfer` type, `transactions` account columns + backfill |
| `supabase/migrations/20260101000004_accounts_rls.sql` | Row Level Security (`accounts`) |
| `supabase/seed/99_demo_users.sql` | Two demo accounts + sample transactions |
| `supabase/apply_all.sql` | Generated by `dart tool/gen_apply_all.dart`. Do not edit. |
...
Four tables. Everything is private to one user.
...
auth.users ──1:1── profiles ──1:N── transactions
                       ...
                       │                  └── raw_email_id ──0:1──┐
                       ├──1:N── raw_emails ───────────────────────┘
                       └──1:N── accounts ──1:N── transactions (account_id)
```
...
| Type | Values | Used by |
|---|---|---|
| `transaction_type` | `income`, `expense`, `transfer` | `transactions.type` |
| `transaction_source` | `manual`, `email` | `transactions.source` |

The UI labels `income` as **Pemasukan**, `expense` as **Pengeluaran**, and
`transfer` as **Transfer**. Transfers move money between two accounts of the
same user and never count toward income/expense totals: every aggregate query
filters `type in ('income', 'expense')`.
...
| `account_id` | `uuid` | → `accounts(id)`, `on delete set null`. Owning account; SOURCE for transfers |
| `transfer_to_account_id` | `uuid` | → `accounts(id)`, `on delete set null`. DESTINATION for transfers, else null |
...
## `accounts` — named balance containers ("Sumber Dana")

Balances are **derived, never stored**: income adds, expense subtracts,
transfer-out subtracts, transfer-in adds (see `SourceAccount.balances`).
A stored balance column would drift from the ledger.

| Column | Type | Notes |
|---|---|---|
| `id` | `uuid` PK | |
| `user_id` | `uuid` | → `profiles(id)`, `on delete cascade` |
| `name` | `text` | unique per user. Pre-existing rows backfill to `Lainnya` |
| `kind` | `text` | `bank` / `e-wallet` / `cash` / `other` (free text, not an enum) |
| `icon` | `text` | emoji/glyph, e.g. `🏦` |
| `color` | `text` | hex string, e.g. `#7C5CFF` |
| `created_at`, `updated_at` | `timestamptz` | |
...
- Row Level Security: all four tables are per-user. `profiles` has select/insert/
  update; `transactions`, `raw_emails`, and `accounts` have all four verbs, each
...
| `data/source/source_account.dart` | `accounts` |
| `data/model/history.dart` | maps `transactions` |
| `data/model/account.dart` | maps `accounts` |
